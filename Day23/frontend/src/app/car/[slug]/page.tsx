...
export default function CarAuctionDetailPage() {
  ...
  const Lightbox = () => {
    ...
    const images =
      car.images?.length > 0 ? car.images : ["/placeholder-car.webp"];
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Deduplicate bids just in case the backend returns duplicates
        const uniqueBids = Array.from(
          new Map(bidsRes.data.map((bid: any) => [bid._id, bid])).values(),
        );

        setCar(carRes.data);
        setBids(uniqueBids);
        setBidAmount(
          Math.max(carRes.data.currentBid, carRes.data.startingBid) + 100,
        );
      } catch (err) {
  ...
  useEffect(() => {
    if (socket && carId) {
      ...
      socket.on("newBid", (newBid: any) => {
        setBids((prev) => {
          const exists = prev.find((b) => b._id === newBid._id);
          if (exists) {
            return [newBid, ...prev.filter((b) => b._id !== newBid._id)];
          }
          return [newBid, ...prev];
        });
        setCar((prev: any) => ({
          ...
          bidCount: (prev?.bidCount || 0) + (newBid.isNewBid ? 1 : 0), // Backend should ideally tell us if it's new
        }));
  ...
  if (!car)
    return (
      <div className="p-20 text-center text-2xl font-bold">Car not found.</div>
    );
  ...
  const isOwner =
    ...
    ((typeof car.uploadedBy === "string" &&
      car.uploadedBy === (user._id || user.id)) ||
      ...
      <div className="container mx-auto px-4 lg:px-[118px] -mt-8 relative z-10">
        ...
        <div className="bg-white rounded-b-[10px] p-6 lg:p-10 shadow-xl flex flex-col gap-10">
          ...
          <div className="flex flex-col gap-6">
            ...
            {/* Desktop: 1+6 Grid */}
            <div className="hidden lg:flex gap-4 h-[400px] xl:h-[550px]">
              {/* Featured View (Left) */}
              <div
              ...
              >
                ...
                <Image
                  src={
                    car.images?.[activeImageIndex] || "/placeholder-car.webp"
                  }
                  alt={car.title}
                  fill
              ...
              <div className="flex-1 grid grid-cols-2 grid-rows-3 gap-4">
                ...
                {car.images?.length < 6 &&
                  [...Array(Math.max(0, 6 - (car.images?.length || 0)))].map(
                    (_, i) => (
                      <div
                        key={`empty-${i}`}
                        className="bg-gray-50 rounded-[8px] border-2 border-dashed border-[#EAECF3] flex items-center justify-center"
                      >
                        <Image
                          src="/placeholder-car.webp"
                          alt="Empty"
                          width={40}
                          height={40}
            ...
            <div className="lg:hidden flex flex-col gap-4">
              ...
              <div className="h-20 w-full">
                <Swiper
                  ...
                  className="h-full"
                >
                  {(car.images?.length > 0 ? car.images : []).map(
                    (img: string, i: number) => (
                      <SwiperSlide
                        key={i}
                        onClick={() => setActiveImageIndex(i)}
                        className={`cursor-pointer rounded-lg overflow-hidden border-2 transition-all ${i === activeImageIndex ? "border-[#2E3D83]" : "border-transparent"}`}
                      >
                        <Image
                          src={img}
                          alt="Thumb"
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-7 gap-px bg-[#EAECF3] border border-[#EAECF3] rounded-[10px] overflow-hidden shadow-sm">
            <div className="bg-white p-4 flex flex-col items-center justify-center gap-1">
              ...
              <span
                className={`text-[14px] font-bold ${timeLeft.isExpired ? "text-gray-400" : "text-[#EF233C]"}`}
              >
                {timeLeft.isExpired
                  ? "Ended"
            ...
            <div className="bg-white p-4 flex flex-col items-center justify-center gap-1">
              ...
              <span className="text-[14px] font-bold text-[#2E3D83]">
                {car.auctionEndDate
                  ? new Date(car.auctionEndDate).toLocaleDateString("en-US", {
                      hour: "2-digit",
                      minute: "2-digit",
                    })
                  : "N/A"}
            ...
            <div className="flex-1 flex flex-col gap-12">
              ...
              <div className="flex flex-col gap-6">
                ...
                <div className="text-[#545677] text-[16px] leading-relaxed font-medium mt-4">
                  {car.description ||
                    "No detailed description provided for this vehicle."}
                  <p className="mt-4">
                    Experience sheer driving pleasure with this meticulously
                    maintained {car.title}. Combining luxury with performance,
                    it offers an unparalleled journey every time you sit behind
                    the wheel.
                  </p>
                </div>
              </div>
              ...
              <div className="flex flex-col gap-6">
                <div className="bg-[#2E3D83] h-[50px] rounded-t-[8px] flex items-center px-6 shadow-sm">
                  <h3 className="text-white text-[18px] font-bold uppercase tracking-tight">
                    Top Bidder
                  </h3>
                </div>
                <div className="bg-[#F1F2FF] rounded-b-[8px] p-6 md:p-8 border border-[#EAECF3] flex flex-col md:flex-row gap-8 lg:gap-10 items-center">
                  <div className="relative w-[140px] h-[140px] rounded-full overflow-hidden border-4 border-white shadow-xl flex-shrink-0 bg-gray-50">
                    <Image
                      src={
                        topBid?.user?.profilePicture ||
                        `https://api.dicebear.com/7.x/avataaars/svg?seed=${topBid?.user?.firstName || "User"}`
                      }
                      alt="Top Bidder"
                      fill
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 flex-1 gap-x-12 gap-y-6 w-full">
                    <div className="flex flex-col gap-1">
                      ...
                      <span className="text-[16px] text-[#545677] font-semibold">
                        {topBid
                          ? `${topBid.user.firstName} ${topBid.user.lastName}`
                          : "No bids yet"}
                      </span>
                    </div>
                    <div className="flex flex-col gap-1">
            ...
            <div className="w-full lg:w-[320px] flex flex-col gap-6">
              ...
              <div className="bg-[#F1F2FF] rounded-[10px] p-6 border border-[#EAECF3] shadow-sm">
                <div className="flex justify-between items-center mb-6">
                  <div className="flex flex-col">
                    <span className="text-[12px] font-bold text-[#2E3D83] uppercase">
                      Starts From
                    </span>
                    <span className="text-[18px] font-bold text-[#2E3D83]">
                      {formatCurrency(car.startingBid)}
                    </span>
                  </div>
                ...
                <div className="relative h-2 bg-white rounded-full mb-8 shadow-inner border border-gray-100">
                  <div
                    ...
                    style={{
                      width: `${Math.min(((car.currentBid - car.startingBid) / car.startingBid) * 100 + 10, 100)}%`,
                    }}
                  />
                  <div
                    ...
                    style={{
                      left: `calc(${Math.min(((car.currentBid - car.startingBid) / car.startingBid) * 100 + 10, 100)}% - 12px)`,
                    }}
                  >
                ...
                <div className="flex flex-col gap-6">
                  {car.status === "live" ? (
                    <>
                      {!isOwner ? (
                        <>
                            ...
                            <div className="flex items-center gap-3 bg-white border border-[#EAECF3] p-1 rounded-[5px] shadow-sm">
                              <button
                                onClick={() =>
                                  setBidAmount((prev) =>
                                    Math.max(
                                      car.currentBid + car.minIncrement,
                                      prev - car.minIncrement,
                                    ),
                                  )
                                }
                              ...
                              <button
                                onClick={() =>
                                  setBidAmount(
                                    (prev) => prev + (car.minIncrement || 100),
                                  )
                                }
                                className="w-8 h-8 flex items-center justify-center text-[#2E3D83] hover:bg-gray-100 rounded transition-colors"
                              >
                          ...
                          {error && (
                            <p className="text-red-500 text-xs text-center -mb-4 animate-shake">
                              {error}
                            </p>
                          )}
                        ...
                        <div className="bg-[#2E3D83]/5 border-2 border-dashed border-[#2E3D83]/20 rounded-[8px] p-6 text-center">
                          ...
                          <h4 className="text-[16px] font-bold text-[#2E3D83] uppercase tracking-tight mb-2">
                            Seller Control
                          </h4>
                          <p className="text-[12px] text-[#545677] font-medium leading-relaxed">
                            You are the owner of this vehicle. Monitor the bid
                            activity below and manage your listing from the
                            dashboard.
                          </p>
                          <button
                            onClick={() =>
                              router.push("/my-profile?tab=my-cars")
                            }
                    ...
                    <div className="bg-[#2E3D83]/5 border border-[#2E3D83]/20 rounded-[8px] p-6 text-center">
                      <h4 className="text-[18px] font-bold text-[#2E3D83] uppercase tracking-tight mb-2">
                        Auction Ended
                      </h4>
              ...
              <div className="flex flex-col bg-white border border-[#EAECF3] rounded-[10px] overflow-hidden shadow-sm">
                <div className="bg-[#2E3D83] h-[60px] flex items-center px-6">
                  <h3 className="text-white text-[16px] font-bold uppercase tracking-tight">
                    Bidders List
                  </h3>
                </div>
                <div className="p-2 flex flex-col divide-y divide-[#EAECF3]">
                  {bids.length === 0 ? (
                    <div className="p-8 text-center text-[#A0A0A0] italic text-sm">
                      No bids yet. Start the auction!
                    </div>
                  ) : (
                    bids.map((bid, i) => (
                      <div
                        key={bid._id}
                        className="p-4 flex items-center justify-between hover:bg-[#F1F2FF] transition-colors rounded-lg group"
                      >
                        <div className="flex flex-col">
                          <span
                            className={`text-[14px] font-bold ${i === 0 ? "text-[#2E3D83]" : "text-[#545677]"}`}
                          >
                            {bid.user
                        ...
                        <span
                          className={`text-[16px] font-bold ${i === 0 ? "text-[#EF233C]" : "text-[#2E3D83]"}`}
                        >
